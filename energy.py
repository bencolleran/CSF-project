...
def genealog(lst):#convert an on vector to a genealogical spin coupling
    n=0
    lst1=[]
...
def matrix_element(i,j):    
    E=(np.sum(np.dot(get_mc_one_rdm(coupled_states(i,1)[0],coupled_states(j,1)[0],coupled_states(i,1)[1],coupled_states(j,1)[1]),single_electron_matrix)))
    +0.5*np.sum(np.dot(get_mc_two_rdm(coupled_states(i,2)[0],coupled_states(j,2)[0],coupled_states(i,2)[1],coupled_states(j,2)[1]), double_electron_tensor))
    return E

print(matrix_element(1,0))

print(np.sum(np.dot(get_mc_two_rdm(coupled_states(0,2)[0],coupled_states(0,2)[0],coupled_states(0,2)[1],coupled_states(0,2)[1]), double_electron_tensor)))
print(np.einsum("pqrs,pqrs",get_mc_two_rdm(coupled_states(0,2)[0],coupled_states(0,2)[0],coupled_states(0,2)[1],coupled_states(0,2)[1]), double_electron_tensor,optimize='optimal'))
'''
...
test_task=[(i,j) for i in range(2) for j in range(2)]

for result in range(4):
    i, j, modified_value = process_element(test_task)
