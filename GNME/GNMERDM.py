...
from GNME.GNMEAuxiliary import lowdin_pairing, get_weighted_codensity, get_unweighted_codensity, get_parity
...
def get_three_rdm_contrib(ddet, dmo, cdet, cmo, sao, ao2mo):
    r"""
    Get contributions to the 3-RDM for two non-orthogonal determinants (ddet and cdet), each with their MO coefficients
    (dmo, cmo). 'd' labels the bra state and 'c' labels the ket state
    :param ddet:
    :param dmo:
    :param cdet:
    ...
    # Return zero matrix if m > 3
    if m > 3:
        return np.zeros((nmos, nmos, nmos, nmos, nmos, nmos))
    elif m == 0:  # There is 1 way of partitioning the (lack of) zeroes
        fcontraction = np.einsum("sp,tq,ur->pqrstu", M, M, M, optimize='optimal')  # Define fundamental contraction
        return antisymmetrise_contraction(fcontraction) * redS
    elif m == 1:  # There are 3 ways of partitioning "1"
        fcontraction = np.einsum("sp,tq,ur->pqrstu", M, M, P, optimize='optimal') + \
                       np.einsum("sp,tq,ur->pqrstu", M, P, M, optimize='optimal') + \
                       np.einsum("sp,tq,ur->pqrstu", P, M, M, optimize='optimal')
        return antisymmetrise_contraction(fcontraction) * redS
    elif m == 2:  # There are 3 ways of partitioning "0"
        fcontraction = np.einsum("sp,tq,ur->pqrstu", M, P, P, optimize='optimal') + \
                       np.einsum("sp,tq,ur->pqrstu", P, P, M, optimize='optimal') + \
                       np.einsum("sp,tq,ur->pqrstu", P, M, P, optimize='optimal')
        return antisymmetrise_contraction(fcontraction) * redS
    else:  # m == 3
        fcontraction = np.einsum("sp,tq,ur->pqrstu", P, P, P, optimize='optimal')  # Define fundamental contraction
        return antisymmetrise_contraction(fcontraction) * redS


def get_four_rdm_contrib(ddet, dmo, cdet, cmo, sao, ao2mo):
    r"""
    Get contributions to the 4-RDM for two non-orthogonal determinants (ddet and cdet), each with their MO coefficients
    (dmo, cmo). 'd' labels the bra state and 'c' labels the ket state
    :param ddet:
    :param dmo:
    :param cdet:
    ...
    # Return zero matrix if m > 4
    if m > 4:
        return np.zeros((nmos, nmos, nmos, nmos, nmos, nmos, nmos, nmos))
    elif m == 0:  # There is 1 way of partitioning the (lack of) zeroes
        fcontraction = np.einsum("wp,vq,ur,ts->pqrstuvw", M, M, M, M, optimize='optimal')  # Define fundamental contraction
        return antisymmetrise_contraction(fcontraction) * redS
    elif m == 1:  # There are 4 ways of partitioning "1"
        fcontraction = np.einsum("wp,vq,ur,ts->pqrstuvw", M, M, M, P, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", M, M, P, M, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", M, P, M, M, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", P, M, M, M, optimize='optimal')
        return antisymmetrise_contraction(fcontraction) * redS
    elif m == 2:  # There are 6 ways of partitioning 2 * "0"
        fcontraction = np.einsum("wp,vq,ur,ts->pqrstuvw", M, M, P, P, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", M, P, P, M, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", P, P, M, M, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", P, M, P, M, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", M, P, M, P, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", P, M, M, P, optimize='optimal')
        return antisymmetrise_contraction(fcontraction) * redS
    elif m == 3:
        fcontraction = np.einsum("wp,vq,ur,ts->pqrstuvw", P, P, P, M, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", P, P, M, P, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", P, M, P, P, optimize='optimal') + \
                       np.einsum("wp,vq,ur,ts->pqrstuvw", M, P, P, P, optimize='optimal')
        return antisymmetrise_contraction(fcontraction) * redS
    else:  # m == 4
        fcontraction = np.einsum("wp,vq,ur,ts->pqrstuvw", P, P, P, P, optimize='optimal')  # Define fundamental contraction
        return antisymmetrise_contraction(fcontraction) * redS


def get_gnme_rdm(dets, coeffs, mos, ref_mo, sao, degree):
    r"""
    ...
    if len(mos) == 1:
        ...
        if degree == 1:
            ...
            for i in range(n):
                for j in range(n):
                    rdm += get_one_rdm_contrib(dets[i], mos[0], dets[j], mos[0], sao, ao2mo) * coeffs[i] * coeffs[j] * \
                           dets[i][0] * dets[j][0]
            return rdm
        ...
        if degree == 2:
            ...
            for i in range(n):
                for j in range(n):
                    rdm += get_two_rdm_contrib(dets[i], mos[0], dets[j], mos[0], sao, ao2mo) * coeffs[i] * coeffs[j] * \
                           dets[i][0] * dets[j][0]
            return rdm

        if degree == 3:
            rdm = np.zeros((ndims, ndims, ndims, ndims, ndims, ndims))
            for i in range(n):
                for j in range(n):
                    rdm += get_three_rdm_contrib(dets[i], mos[0], dets[j], mos[0], sao, ao2mo) * coeffs[i] * coeffs[j] * \
                           dets[i][0] * dets[j][0]
            return rdm

        if degree == 4:
            rdm = np.zeros((ndims, ndims, ndims, ndims, ndims, ndims, ndims, ndims))
            for i in range(n):
                for j in range(n):
                    rdm += get_four_rdm_contrib(dets[i], mos[0], dets[j], mos[0], sao, ao2mo) * coeffs[i] * coeffs[j] * \
                           dets[i][0] * dets[j][0]
            return rdm
    else:
        ...
        if degree == 3:
            ...
            for i in range(n):
                for j in range(n):
                    rdm += get_three_rdm_contrib(dets[i], mos[i], dets[j], mos[j], sao, ao2mo) * coeffs[i] * coeffs[j] * dets[i][0] * dets[j][0]
            return rdm
        ...
        if degree == 4:
            ...
            for i in range(n):
                for j in range(n):
                    rdm += get_four_rdm_contrib(dets[i], mos[i], dets[j], mos[j], sao, ao2mo) * coeffs[i] * coeffs[j] * dets[i][0] * dets[j][0]
            return rdm
