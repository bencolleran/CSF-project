...
def perms(n,m):
    ...
    lst2=[lst1[j] for j in range(len(lst1)) if constraints_singly(lst1[j],(nmos-sum(nums[n])))]
...
def single_site_csf(csf,m):
    coeffs=[get_total_coupling_coefficient(genealog(perms(5,m)[a]), CSF.get(csf)) for a in range(len(perms(5,m)))]
    kets=perms(5,m)
    csf=[kets]+[coeffs]
    return csf

def double_site_csf(csf,m):
    coeffs=[get_total_coupling_coefficient(genealog(perms(6,m)[a]), CSF.get(csf)) for a in range(len(perms(6,m)))]
    kets=perms(6,m)
    csf=[kets]+[coeffs]
    return csf
...
#print(annihilate(3,copy.deepcopy(ket)))
#print(annihilate(4,annihilate(3,copy.deepcopy(ket))))
#print(create(6,annihilate(4,annihilate(3,copy.deepcopy(ket)))))
#print(create(1,create(6,annihilate(3,annihilate(4,copy.deepcopy(ket))))))
